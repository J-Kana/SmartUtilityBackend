        ...
        MicroRegion.findAll()
            .then(async objects => {
                if (objects.length === 0) {
                    ...
                    let MicroRegionArr = []
                    for(let microregion of objects) {
                        try {
                            let regions = await Region.findByPk(microregion.dataValues.regionId)
                            microregion.dataValues.region = regions
...
exports.getObject = (req, res) => {
    let {id} = req.params
    try {
        MicroRegion.findByPk(id)
            .then(async object => {
                if (!object) {
                    templase(500, "Объект не найден!", [], true, res)
                } // если Объект не найден
                try {
                    let regions = await Region.findByPk(object.dataValues.regionId)
                    object.dataValues.region = regions
...
exports.objectCreate = (req, res) => {
    try {
        let {body} = req
        MicroRegion.create(body).then(object=>{
            if (!object){
